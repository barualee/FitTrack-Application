...
def prediction(request):
    ...
    if request.method =="POST":
        ...
        try:
            ...
            data=pd.read_csv(r'C:\Users\Dell\Downloads\accelerometer_data.csv')
            # print(data)
            data.dropna(inplace=True)
            ...
            data['age'] = 26
            data['gender'] = 1.0
            timediff=data['Timestamp'].diff().total_seconds()
            time_diff =  data['Timestamp'].diff().mode()[0].dt.total_seconds()
            sampling_rate = 1 / time_diff
            # freq=1/data['Timestamp'].diff().median().total_seconds()
            print(sampling_rate)
